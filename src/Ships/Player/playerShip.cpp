...
Player::Player(int Xposition, int Yposition){
    ...
    this->shipShield.load("CompressedImages/ForceShield.png");
    this->secondShipSprite.load("CompressedImages/secondShip.png");
...
void Player::draw() {
    ...
    // If the shield is active, draw it around the ship
    if (immunity) {
        this->shipShield.draw(-50,-50,100,100);
    }
    ...
    // End transformation context
    ofPopMatrix();
    
    // Draw the hitbox around the player ship for testing purposes
    if (showHitbox) {
        this->hitBox.draw();
    }
}
...
void Player::shoot() { 
    ...
    // Check if enough time has passed since the last shot
        if (currentTime - lastShotTime >= shotCooldown) {
                if (bossKilled){
                    Projectiles p = Projectiles(ofPoint(this->pos.x, this->pos.y), this->shipOrientation, 25);
                    
                    p.setColors(ofColor::darkGreen, ofColor::darkTurquoise);
...
void Player::addPressedKey(int key) {
    ...
    cout << "Pressed key code: " << key << endl;
    // check if left shift is pressed to enable sprinting
    if (key == 1) {
        holdSprint(true);
    }
}
...
void Player::processPressedKeys() {
    ...
    if(keyMap['q']) movement('q');
    if(keyMap['e']) movement('e');
...
void Player::removePressedKey(int key) {
    key = tolower(key);
    keyMap[key] = false;
    // check if left shift is released to disable sprinting
    if (key == 1) {
        holdSprint(false);
    }
}
...
void Player::movement(char keyPressed) {
    ...
    // adjust acceleration for sprinting
    
    accelerationAmount = (isSprinting ? 10.0 : 5.0);
    
    if (keyPressed == 'q' ) {
        hasShield = true; // Toggle shield state       
    }

    if (keyPressed == 'e'){
        if(hasBomb == true){
            hasBomb = false;
            Kaboom = true; //Bomb Kaboom
        }
...
void Player::holdSprint(bool enableSprint) {

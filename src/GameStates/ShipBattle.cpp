...
ShipBattle::ShipBattle() {
    ...
    life1.load("CompressedImages/1live.png");
    lives2.load("CompressedImages/2lives.png");
    lives3.load("CompressedImages/3lives.png");
    lives0.load("CompressedImages/0lives.png");
...
void ShipBattle::update() {
    ...
    shieldTimer = EnemyManager::getShieldTimer();
...
void ShipBattle::draw() {
    ...
    ShieldTimer(this->shieldTimer, 100);
    
    //Draw a mini box for the bomb. Make sure to draw the bomb inside this box.
        ofNoFill();
        ofDrawRectangle(ofGetWidth() - 150, 30, 50, 50);
        ofFill();
        if (player-> hasBomb){ ///Draw the bomb
            bomb.draw(ofGetWidth() - 150, 30, 50, 50);

        }

        switch(lives){
            case 1:
            life1.draw(240, 5, 250, 100);
            break;

            case 2:
            lives2.draw(240, 5, 250, 100);
            break;

            case 3:
            lives3.draw(240, 5, 250, 100);
            break;

            default:
            lives0.draw(240,5,250,100);
            break;
        }
...
void ShipBattle::ShieldTimer(int currShield, int maxShield) { ///Bar For Shield
    indicatorFont.drawString("SHIELD", 10, 120);
    ofNoFill();
    ofDrawRectangle(10, 130, maxShield, 10);
    ofFill();
    ofSetColor(ofColor::blue);
    if (currShield <= 100) {
    ofDrawRectangle(10, 130, currShield, 10);
    }
    ofSetColor(ofColor::white);
}
...
void ShipBattle::reset(){
    ...
    this->player->bossKilled = false;
    this->player->hasBomb = false;
    EnemyManager::setShieldTimer(0);
    EnemyManager::cleanUp();
    EnemyManager::resetKillSpreeTimer(0);
    EnemyManager::resetBossWarningTimer(0);
    setFinished(false);
    setNextState("");

...
// Define static members here
    ...
    bool EnemyManager::sakuyaSeen = false;
    ...
    int EnemyManager::shieldDuration = 0; 
    int EnemyManager::shieldTimer = 0;
...
void EnemyManager::manageCollisions(Player* player) {
  ...
  // Handle collisions between player bullets and enemies
    ...
    // Handle collision with enemies and bosses 
    for (auto& enemy : enemyList) {
        if (enemy->getHitBox()->isColliding(player->hitBox)) {
            if (!player->immunity) {
    ...
    for (auto& boss : bossList) {
        //showHitboxes(true);
        if (boss->getHitBox()->isColliding(player->hitBox)) {
            if (!player->immunity) {
            player->health = max(player->health - 0.5, 0.0);
            }
        }
    }
    ...
    for (auto& Boss : bossList) {
        ...
        for (auto& bullet : player->bullets) {
            ...
            if (!bullet.bulletIsOutOfBounds() && Boss->getHitBox()->isHit(bullet)) {
                ...
                if (player-> Kaboom){
                   Boss->takeDamage(Boss->getBossHealth()/2);
                ...
                if (Boss->isDead()) {                   //If the boss has died from a bullet
                    ...
                    resetKillSpreeTimer(150);
                    shieldTimer = 150;
                    player-> hasBomb = true; //Gives Bomb to player
...
bool EnemyManager::isShieldReady() {
    if (shieldTimer >= 100) { 
        shieldTimer = 100;
        return true;
    }   
...
int EnemyManager::getShieldTimer() {
    return shieldTimer;
}
void EnemyManager::setShieldTimer(int value) {
    shieldTimer = value;
}
...
void EnemyManager::spawnEnemy(Player* player){
    ...
    if (enemySpawnTimer >= spawnInterval) {
        ...
        if (!bossIsActive) { // Ensure no boss is currently active before spawning another
            if (currentScore > 10000 && !sakuyaSeen) { //changed number to see boss rapidly, it should be to 75,000 pointws
                // Spawn Sakuya Izayoi
                initiateBossSpawn("Sakuya Izayoi");
                sakuyaSeen = true;
            }
        ...
        if (enemyList.size() <= 6) {
            ...
            if(currentScore > 5000) {
                enemyList.push_back(make_unique<GalgolaEnemy>(spawnLocation.x, spawnLocation.y));//Check, spawn Galgola at 5000 player score
            }
            if (currentScore >= 0) {
...
void EnemyManager::spawnBoss(const string& bossType) {
    ...
    else if (bossType == "Sakuya Izayoi") {
        sakuyaSeen = true;
        auto boss = make_unique<Sakuya>(ofGetWidth()/2,50,"Sakuya Izayoi ORT");
        bossList.push_back(move(boss));
    }
...
int EnemyManager::whichSpawnInterval(int playerScore) {
    ...
    if (!bossIsActive && sakuyaSeen) return 10; //Do we need this?
